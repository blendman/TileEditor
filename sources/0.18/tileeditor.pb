...
Procedure CreateTheTile(x, y)
  ...
  If StartDrawing(SpriteOutput(sp))
    ...
    DrawImage(ImageID(#image_Tileset), -tileX * tileW, -tileY * tileH)
    StopDrawing()
  EndIf
...
Procedure WinPixelArtPaint()
  ...
  w1=SetMin(WinW -200, 600)
	...
	If OpenWindow(#WinPixelArtPaint,0,0,w1,h1,Lang("Pixel Art Paint"), #PB_Window_SystemMenu|#PB_Window_ScreenCentered, WindowID(#WinMain))
		...
		ImageW = 16
		ImageH = 16
		GrabImage(#image_Tileset, #Image_PAPA, tilex*tilew, tiley*tileH, tilew, tileH)
		...
		brushcolor = 0
		brushalpha = 255
		color = RGBA(Red(brushcolor), Green (brushcolor),Blue(brushcolor),brushalpha)
    PAP_UpdateCanvas()
				...
				Case #PB_Event_Gadget
				  Select EventGadget
				    Case #G_WinPAP_ToolAction
				      brushAction=GetGadgetState(EventGadget)
				      Select brushAction
				        Case 1
				          If OldAlpha <> BrushAlpha
    				        OldAlpha = BrushAlpha
    				      EndIf
    				      BrushAlpha = 0
    				      color = RGBA(Red(brushcolor), Green (brushcolor),Blue(brushcolor),0)
    				    Case 0 ; paint
    				      If OldAlpha <> BrushAlpha
    				        BrushAlpha = OldAlpha
    				      EndIf
				    ...
				    Case #G_WinPAP_Toolsize
				    	brushsize=GetGadgetState(EventGadget)
				    Case #G_WinPAP_Tooltype
				    	brushtype=GetGadgetState(EventGadget)
				    Case #G_WinPAP_Toolalpha
				      brushalpha=GetGadgetState(EventGadget)
				      color = RGBA(Red(brushcolor), Green (brushcolor),Blue(brushcolor),brushalpha)
				    ...
				    Case #G_WinPAP_MainCanvas
				      ...
				      If EventType = #PB_EventType_LeftButtonDown Or (EventType = #PB_EventType_MouseMove And GetGadgetAttribute(cnv, #PB_Canvas_Buttons) & #PB_Canvas_LeftButton)
				        ...
				        EndIf
				      Else
				        ; Debug "autre eventtype : "+Str(EventType)
				        Select EventType
...
; CursorPosition = 76
; Folding = CAPAAAAAAAA5BAAABAIAAAAAAAAACA9-BAQA5
; EnableXP