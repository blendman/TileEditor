...
; - menu edit : set properties (x,y, layer, visible, lock, tileid, tileset)
...
Global TileW=32, TileH=32, TileID, TileX, TileY, TileSetW, TileSetH
...
Structure sLayer
  Array Tile.sTile(0)
EndStructure
Global MapW =20, MapH =20, LayerId, TileID, tileID_byMouse
...
Procedure CreateTheTile(x, y)
  ...
  If TileID > ArraySize(layer(layerID)\Tile())
    ReDim layer(layerID)\Tile(TileId)
  EndIf
...
Procedure Doc_Save(saveas=0)
  ...
  If name$<>""
    If OpenFile(0, "Test.txt")
      d$ = ","
      ; add information
      WriteStringN(0, "info,"+#VersionProgram+d$)
      ; +FormatDate("%dd/%mm/%yyyy", Date())+d$
      
      ; the Map
      WriteStringN(0, "map,"+Str(MapW)+d$+Str(MapH)+d$)
      ...
      For i = 0 To ArraySize(Layer())
        For j = 0 To ArraySize(layer(i)\Tile())
          With Layer(i)\Tile(j)
            If IsSprite(\sprite)
              info$ = "tile,"+Str(\x)+d$+Str(\y)+d$+\imagename$+d$+Str(\tileid)+d$+Str(\layer)+d$+Str(\usealpha)+d$+Str(\lock)+d$+Str(\visible)+d$+Str(\type)+d$
              WriteStringN(0, info$)
            EndIf
          EndWith
        Next
...
If OpenWindow(0, 0, 0, w, h, "Tile Editor "+#VersionProgram, 
  ...
  If CreateMenu(0, WindowID(0))
    MenuTitle(Lang("File"))
    MenuItem(#Menu_FileOpen, Lang("Open a document"))
    MenuItem(#Menu_FileMerge, Lang("Merge a document"))
    MenuBar()
    ...
    MenuItem(#Menu_ToolAddTile, Lang("Add a tile")   +Chr(9)+"A")
    MenuItem(#Menu_ToolDeleteTile, Lang("Delete a tile")  +Chr(9)+"D")
        ...
        If x>=0 And y>=0 And x<=MapW And y<=Maph
          ...
          Select action
            Case #Action_AddTile, #Action_ChangeTile
              If Layer(LayerID)\Tile(TileID)\sprite <= 0  Or action = #Action_ChangeTile
                CreateTheTile(x, y)
              Else
